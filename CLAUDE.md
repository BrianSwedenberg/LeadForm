...
- Three implementations exist on separate branches:
  - `feature/zip-filter-bundle` — client-side Set ✅ COMPLETE
...
## Bundle Branch Notes (`feature/zip-filter-bundle`)

- `src/data/allowedZips.ts` is the source of truth for the allowed zip list on this branch
- To add a zip: add the string to the `ALLOWED_ZIPS` Set in that file and redeploy
